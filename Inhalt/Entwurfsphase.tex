...
Als Programmiersprache wurde \acused{PHP}\footnote{vgl. \cite{PHP}} gewählt, da dies bereits Grundlage zahlreicher Anwendungen ist, somit leichter an wy-connect angebunden werden kann und die Entwickler wycomcos mit dieser Sprache bereits vertraut sind. 
...
Das Laravel Passport Paket bringt eine beispielhafte Oberfläche mit, in der alle relevanten Anforderungen Anwendung finden. Screenshots dazu sind unter \fullnameref{app:screens} zu sehen.
...
Nachdem der Benutzer sich am Autorisierungsserver eingeloggt hat, sieht er die Autorisierungsaufforderung. Wenn der Nutzer auf \flqq Zulassen\frqq  klickt, leitet der Dienst ihn mit einem Autorisierungscode zurück zum Client. Ein Beispiel für eine Antwort wäre \texttt{https://wy-connect-client.test/login/wyconnect/callback} mit folgenden Parametern:
